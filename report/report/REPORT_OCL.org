#+TITLE: Histogram Equalization in PyOpenCL and NumPy
...
We here present the problem, discuss the techical decision taken throughout its implementation, show our implementation, and its performance with respect to a cpu computed baseline implemented in ~NumPy~
* Histogram Equalization
** Problem Statement
[[/home/nil/Pictures/Screenshots/Wayland/cropped/26-09-11--10-18-42-275894119.png]]
...
** General Procedure, and Parallel Patterns Therein
...
The project has been therefore implemented using the ~OpenCL~ API, as it was the closes in API and performance characteristics to CUDA.
The project had initially been developed using the ~PyOpenCL~ library, with plans to later port it to C and have it directly use ~libOpenCL~, this plan was later scrapped and the ~PyOpenCL~ version was deemed fit as a final artefact.

Despite the choice of ~OpenCL~, over ~CUDA~, the remainder of this report shall still use ~CUDA~ terminology (thread, thread block, grid, ...) over the corresponding ~OpenCL~ terminology (work item, work group, ndrange, ...).

Due to a software update breaking ~OpenCL~ support on the author's machine the project has been also developed using ~docker~ to maintain an environment with working drivers for the author's hardware.

*** ~NumPy~ 
Out of fear that python runtime overhead could introduce a bias in performance measurements, the planned C CPU baseline impelmentation has instead been developed using ~NumPy~, so that any runtime overhead between the GPU and CPU versions of the program would have been equal for both sides.

** PyOpenCL Implementation
The ~PyOpenCL~ version of the code has been laid out according to the following pipeline
- Host image is read from file, host histogram is initialized to all zeros
- Device image and histogram are initialized and the host image and histogram values are copied therein (initialized as ~OpenCL~ buffers with the ~COPY_HOST_PTR~ flag)
- Device image is converted from RGB to HSL inplace (one kernel launch)
- Histogram of L channel in device image computed into device histogram (one kernel launch)
- Addition scan of histogram is computed inplace to produce cumulative histogram (heirarchical scan, 3 kernel launches)
- Device image L channel normalized inplace (one kernel launch)
- Device image converted from HSL back into RGB (one kernel launch)
...
The full ~OpenCL~ C code for the conversion from rgb to hsl is here provided for illustrative purposes
...
- Image is split into R, G, and B channels (transposed first then split into 3 cells of transposed array)
...
- the normalized R, G, and B channels are then discretized back into ~np.uint8~ and used to construct the normalized image (requires another transposition)

As hinted to above this is a different set of operations from the ~PyOpenCL~ code, but porting the degree of index fiddling done in OpenCL to ~NumPy~ proved problematic and not too idiomatic, leading to the two transpositions instead. Given how both transpositions are \(O(n)\) this adds a lot of overhead to the ~NumPy~ version, as we shall see in the benchmarks.
...
- a flat red, square image
- an HD image of the Mona Lisa
- a grayscale photograph
- a color photograph of a landscape
- a color photograph of italian journalist Germano Mosconi
...
** Conclusions
The ~OpenCL~ version has been found to be significantly faster than the ~NumPy~ versions on most images, achieving a speedup of around 40 for all but one tested images.

The 1.6 speedup for the flat red image is likely due to a combination of factors such as
- Small size of the image, making the cost of transfer between host and device quite expensive relative to work done on the device side
- Degenerate histogram of the image, leading to high contention while computing image histogram, likely slowing gpu implementation down further
